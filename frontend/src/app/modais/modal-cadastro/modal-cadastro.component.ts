...
export class ModalCadastroComponent implements OnInit {
  ...
  ngOnInit(): void {   
    this.getEstados(() => {
      if (this.dadosUsuario) {
        this.usuario.Nome = this.dadosUsuario.Nome;
        this.usuario.Sexo = this.dadosUsuario.Sexo;
        this.usuario.Email = this.dadosUsuario.Email;
        this.usuario.Estado = this.dadosUsuario.Estado;
        this.usuario.Cidade = this.dadosUsuario.Cidade;

        this.estadoSelecionado = this.estados.find(e => e.sigla === this.dadosUsuario.Estado) ?? {} as Estado;
        if (this.estadoSelecionado.sigla) {
          this.getCidades(this.estadoSelecionado.sigla, () => {
            this.cidadeSelecionada = this.cidades.find(c => c.nome === this.dadosUsuario!.Cidade) ?? {} as Cidade;
          });
        }
      }
    });
  ...
  onConfirm() {
    ...
    if(this.dadosUsuario) {
      this.cadastroService.updateUsuario(this.usuario).subscribe({
  ...
  getCidades(uf: string, callback?: () => void) {
    this.loading = true;
